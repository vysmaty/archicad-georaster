...
The compatibility layer owns menu registration, database operations, Survey Point access, Undo scopes and `API_PictureType` creation. Worksheet creation is intentionally outside Undo; picture creation is one Undo operation. On failure, the original database is restored before the empty Worksheet is deleted.
